...
public interface ArticleMapper {
    ...
    @Insert("INSERT TO article (title, content, mid, createtime,updatetime,modifier) " +
            "VALUES (#{title},#{content},#{mid},#{createtime},#{updatetime},#{modifier})")
    @Options(useGeneratedKeys = true, keyColumn = "id", keyProperty = "id")
    void addArticle(ArticleDO articleDO);

    @Update("UPDATE article SET title = #{title},content=#{content}," +
            "mid=#{mid},createtime=#{createtime},updatetime=#{updatetime}," +
